...
RHIFence::RHIFence(const std::shared_ptr<RHIDevice> &device): m_pDevice(device) {
    ...
    vkCreateFence(m_pDevice->GetHandle(), &fenceCreateInfo, nullptr, &m_pFence);
    LOG_INFO("Fence created");
}

RHIFence::~RHIFence() {
    vkDestroyFence(m_pDevice->GetHandle(), m_pFence, nullptr);
}

void RHIFence::Wait() const {
    vkWaitForFences(m_pDevice->GetHandle(), 1, &m_pFence, VK_TRUE, UINT64_MAX);
}

void RHIFence::Reset() const {
    vkResetFences(m_pDevice->GetHandle(), 1, &m_pFence);
}
